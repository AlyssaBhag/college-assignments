# 🔥 The Journey of the Goblin Slayer 🔥
### *A 2D Survivor Shooter Inspired by Stardew Valley's Prairie King*

[![Unity](https://img.shields.io/badge/Unity-2022.3-black?logo=unity)](https://unity.com/)
[![C#](https://img.shields.io/badge/C%23-10.0-blue?logo=c-sharp)](https://docs.microsoft.com/en-us/dotnet/csharp/)
[![License](https://img.shields.io/badge/License-Academic-green)]()

---

## 🎮 About The Game

**The Journey of the Goblin Slayer ** is an intense 2D top-down shooter where you battle through waves of enemies in a desperate fight for survival. Inspired by the beloved mini game *Journey of the Prairie King* from Stardew Valley, this action-packed adventure tests your reflexes, strategy, and determination as the difficulty escalates with each passing wave. You are armed with nothing but your bow and determination, you must face increasingly challenging waves of enemies. Collect powerful perks, upgrade your arsenal, and prove your worth as the ultimate survivor. Will you climb to the top of the leaderboard, or fall to your enemies?

---

## 🎮 How to Play

### Controls
- **WASD / Arrow Keys** - Move your character
- **K** - Shot your weapon
- **ESC** - Pauses/exit game button
- **Spacebar** - Activate special abilities (when available)


## 📸 Screenshots

### 🎮 Start Screen
<div align="center">
  <img src="Screenshots/Start_Page.png" alt="Game Start Screen" width="700">
  <p><em>Welcome to the Journey of the Goblin Slayer!</em></p>
</div>

### ⚔️ Intense Gameplay
<div align="center">
  <img src="Screenshots/Game.png" alt="Gameplay Action" width="700">
  <p><em>Battle through waves of enemies with your bow!</em></p>
</div>

### ⚙️ Game Settings
<div align="center">
  <img src="Screenshots/Settings.png" alt="Settings Menu" width="700">
  <p><em>Customize your gaming experience</em></p>
</div>

---

## 🚀 Future Enhancements

- 🎯 Boss battles at milestone waves
- 🛡️ Additional weapon types and perks
- 🎨 More enemy variants with unique abilities
- 🏆 Achievement system
- 🎵 Enhanced sound effects and music
- 👥 Local co-op multiplayer mode

---

## 📝 Academic Context

This project was developed as part of the **.NET Class** to demonstrate mastery of object-oriented programming concepts in a practical, engaging application. The game showcases clean code architecture, SOLID principles, and real-world software engineering practices.

---

## 🙏 Acknowledgments

- **Team Collaboration**: A huge shout-out to my project partner **Ayhan** for their dedication, creativity, and tireless work in bringing this game to life!
- **Game Inspiration**: *Journey of the Prairie King* mini-game from **Stardew Valley** by [ConcernedApe](https://www.stardewvalley.net/)
- **Visual Assets**: Beautiful pixel art from [Pixel Frog's Tiny Swords Asset Pack](https://pixelfrog-assets.itch.io/tiny-swords)

---

<div align="center">

**Made with 💜 and lots of ☕**
*OOP3 - Durham College - 2026*

</div>  





